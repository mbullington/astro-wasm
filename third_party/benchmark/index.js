...
let freeRequire = typeof require == 'function' && require
...
let rePrimitive = /^(?:boolean|number|string|undefined)$/
...
function runInContext(context) {
    ...
    let doc = isHostType(context, 'document') && context.document

    /** Used to access Wade Simmons' Node.js `microtime` module. */
    let microtimeObject = req('microtime')
    /** Used to access Node.js's high resolution timer. */
    let processObject = isHostType(context, 'process') && context.process

    /** Used to prevent a `removeChild` memory leak in IE < 9. */
    let trash = doc && doc.createElement('div')
    /** Used to integrity check compiled tests. */
    let uid = 'uid' + +performanceNow()
    ...
    function runScript(code) {
        ...
        const originalGlobal = Benchmark
        window.Benchmark = Benchmark
        parent.insertBefore(script, sibling)
        window.Benchmark = originalGlobal
        delete anchor[prop]
    }
    ...
    function filter(array, callback) {
        if (callback === 'successful') {
            ...
            let result = filter(array, 'successful').sort(function (a, b) {
                a = a.stats
                b = b.stats
                return (
    ...
    function cycle(clone, options) {
        ...
        if (clone.running) {
            ...
            if (support.browser && window.Benchmark) {
    ...
    function run(options) {
        ...
        bench.times.timeStamp = +performanceNow()
        bench.emit(event)
