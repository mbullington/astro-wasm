...
// Added by Francisco Martínez
void gpc_set_polygon (Polygon& orig, gpc_polygon *p)
{
	p->num_contours = orig.ncontours ();
	...
	for (unsigned int i = 0; i < orig.ncontours (); i++) {
		p->contour[i].num_vertices = orig.contour (i).nvertices ();
		p->hole[i]= FALSE; /* Assume the contours to be external */
		MALLOC(p->contour[i].vertex, p->contour[i].num_vertices * sizeof(gpc_vertex), "vertex creation", gpc_vertex);
		for (int v = 0; v < p->contour[i].num_vertices; v++) {
			p->contour[i].vertex[v].x = orig.contour (i).vertex (v).x;
			p->contour[i].vertex[v].y = orig.contour (i).vertex (v).y;
		}
	}
}
